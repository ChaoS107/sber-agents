...
2. **handlers.py** - обработка событий
   ...
   - Обработчик голосовых сообщений (voice) → скачивание файла из Telegram → транскрибация через STT (Speech-to-Text) → дальнейшая обработка как текстового сообщения через LLM → сохранение транзакций → показ ответа + статус + баланс
...
3. **llm.py** - интеграция с LLM
   ...
   - Метод `transcribe_audio()` (или аналогичный) - простая обертка над выбранным STT-провайдером (Whisper локально, Yandex SpeechKit или другой сервис), возвращающая текст из голосового сообщения
...
```
Telegram (voice, OGG/OPUS) → handlers.py (скачать файл, при необходимости сконвертировать формат) → 
llm.py / STT-клиент (transcribe_audio: Speech-to-Text, например Whisper локально или облачный сервис) → 
текст → дальше тот же поток, что и для текстового сообщения (get_transaction_response_text, сохранение транзакций, ответ + статус + баланс) → Telegram
```
...
1. Пользователь отправляет голосовое сообщение (voice)
2. Бот скачивает аудиофайл из Telegram (OGG/OPUS, при необходимости конвертирует в нужный формат для STT)
3. Бот отправляет аудио в выбранный STT-провайдер (локальный Whisper или облачный сервис) и получает текстовую транскрипцию
4. Бот передает полученный текст в `get_transaction_response_text()` как обычное текстовое сообщение
5. LLM извлекает транзакции и формирует `TransactionResponse`
6. Бот сохраняет транзакции в `transactions[chat_id]`
7. Бот рассчитывает баланс
8. Бот отправляет пользователю:
   - Ответ LLM из поля `answer`
   - Статус: сколько транзакций найдено и сохранено
   - Текущий баланс
